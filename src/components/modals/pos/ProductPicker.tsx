  ...
  return (
    <div className="p-6 bg-white/5 border border-white/5 rounded-[2rem] space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <select 
          className="w-full px-4 py-3 bg-white/5 border border-white/5 rounded-xl text-white text-[11px] font-bold uppercase outline-none" 
        ...
        <div className="relative" ref={pickerRef}>
          <input
            ...
            className="w-full px-4 py-3 pr-9 bg-white/5 border border-white/5 rounded-xl text-white text-[11px] font-bold uppercase outline-none placeholder:text-gray-500 disabled:opacity-30"
          />
            ...
            <div className="absolute top-full left-0 right-0 mt-2 bg-[#121214] border border-white/10 rounded-xl z-50 max-h-48 overflow-y-auto">
              {filteredProducts.map(prod => (
                <div
                  ...
                  className="px-4 py-3 hover:bg-primary/10 text-xs text-white font-bold border-b border-white/5 cursor-pointer uppercase"
                >
                  {prod.name_uz}
                </div>
              ))}
              {filteredProducts.length === 0 && (
                <div className="px-4 py-3 text-[11px] text-gray-500 font-bold uppercase">{t('not_found', 'Topilmadi')}</div>
              )}
            </div>
          )}
        </div>
      ...
      {p.productStock && p.productStock.isTekstil && (
        <div className="grid grid-cols-2 gap-3">
          <input 
            ...
            className="w-full px-5 py-4 bg-white/5 border border-white/5 rounded-2xl text-white font-bold outline-none" 
            value={p.sellWidth} 
      ...
      {p.productStock && (
        <input 
          ...
          className="w-full px-5 py-4 bg-white/5 border border-white/5 rounded-2xl text-primary font-bold outline-none" 
          value={p.sellPrice} 
      ...
      <button 
        ...
        className="w-full py-4 bg-primary text-black font-bold rounded-2xl uppercase text-[10px] tracking-wide disabled:opacity-30"
      >
