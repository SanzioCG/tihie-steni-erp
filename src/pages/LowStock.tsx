  ...
  return (
    <div className="space-y-5 text-left p-2 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-2">
        <div>
          <h2 className="text-2xl font-bold text-rose-500 uppercase tracking-tighter flex items-center gap-2">
        ...
        <button
          ...
          className="px-6 py-3 bg-white/5 border border-white/10 text-white rounded-xl flex items-center gap-2 hover:bg-white/10 transition-all font-bold text-[10px] uppercase tracking-wide"
        >
      ...
      <div className="relative mx-2">
        ...
        <input 
          ...
          className="w-full pl-14 pr-4 py-5 bg-[#0c0c0e] border border-rose-500/20 rounded-3xl text-white outline-none focus:border-rose-500/40 shadow-xl uppercase font-bold" 
        />
      </div>
      ...
      <div className="bg-[#0c0c0e] border border-rose-500/10 rounded-[2.5rem] overflow-hidden shadow-2xl relative min-h-[300px] mx-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead className="bg-rose-500/5 text-rose-500 border-b border-rose-500/10 uppercase">
              <tr>
                <th className="px-6 py-6 text-[9px] font-bold text-center w-16">#</th>
                <th className="px-6 py-6 text-[9px] font-bold">{t('image')}</th>
                <th className="px-6 py-6 text-[9px] font-bold text-center">{t('batch')}</th>
                <th className="px-6 py-6 text-[9px] font-bold">{t('name')}</th>
                <th className="px-6 py-6 text-[9px] font-bold text-center">{t('qoldiq')}</th>
                <th className="px-6 py-6 text-[9px] font-bold text-right">{t('limit')}</th>
                <th className="px-6 py-6 text-[9px] font-bold text-center">{t('actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/3">
              {filteredData.map((batch: any, index: number) => (
                  <tr key={batch.id} className="group hover:bg-rose-500/2 transition-all">
                    <td className="px-6 py-5 text-gray-700 font-bold text-xs text-center">{index + 1}</td>
                    ...
                    <td className="px-6 py-5 text-center font-mono text-xs text-gray-400">{batch.batch_number}</td>
                    <td className="px-6 py-5 text-sm font-bold uppercase text-white">{batch.products?.name_uz}</td>
                    <td className="px-6 py-5 text-center">
                      <div className={cn(
                        "inline-flex px-4 py-2 rounded-xl font-bold text-sm border transition-all",
                    ...
                    <td className="px-6 py-5 text-right font-bold text-gray-500">{batch.min_limit}</td>
                    <td className="px-6 py-5 text-center">
                      <button 
        ...
        {!loading && filteredData.length === 0 && (
          <div className="py-32 flex flex-col items-center gap-4 text-gray-700">
            ...
            <p className="font-bold uppercase text-xs tracking-wide">{t('no_low_stock')}</p>
          </div>
        )}
      </div>
